...
async def autobot():
    ...
    if who.username:
        username = who.username + "eskaybot"
    else:
        username = "𝗘𝗦𝗞𝗔𝗬" + (str(who.id))[5:] + "ubot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ...
        await bot(EditAdminRequest(int(BOTLOG_CHATID), f"@{username}", rights, "𝗘𝗦𝗞𝗔𝗬 𝗨𝗕𝗢𝗧"))
        ppk = "userbot/resources/logo.jpg"
        await bot(EditPhotoRequest(BOTLOG_CHATID, await bot.upload_file(ppk)))
        heroku_var["BOT_TOKEN"] = token
        heroku_var["BOT_USERNAME"] = f"@{username}"
